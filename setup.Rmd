# Setting up Bookdown {#setup}

The `bookdown` package and [book](https://bookdown.org/yihui/bookdown/get-started.html) is definitely the best way to get started. However, in practice I always find myself copying an existing, working book and modifying it instead of starting from scratch. So this tutorial is going to have you do that as well, using this book as the one you copy from.

[more setup here]

You will have to name your book's repository. To differentiate your book's repo name from this "bookdown-tutorial" repo, here we'll call your book "awesome-book" *but you should consistently name it what you want to name it*.

## Get "bookdown-tutorial" going on your local computer

1. Go to <https://github.com/jules32/bookdown-tutorial>
1. Click the green "clone or download" button and DOWNLOAD ZIP. 
1. Locally on your computer, unzip the folder, save it in a reasonable place
1. Rename 2 things from "bookdown-tutorial" to "awesome-book". You can do this in the finder/windows explorer:
  - the folder itself (that you just unzipped)
  - the .Rproj file
1. Double-click the .Rproj file to launch RStudio 
1. Install packages and restart
  - `install.packages("bookdown")`  
  - `install.packages("usethis")`  
  - Use the menu item Session > Restart R   
1. Click on the Build tab in the top right pane
1. Click on Build Book! 


Nice job! Now let's make it yours, and connect it to GitHub.

## Create your "awesome-book" GitHub repo

1. Go to your GitHub account: github.com/username
1. Click on Repositories, and the green button "New" to create a new repo
1. Name this new repo "awesome-book"
1. DO NOT initialize this repo with a README
1. Click the green "create repository" button — this will take you to your new repo
1. Click the tiny "clone or download" button near the top and COPY URL

## Turn "bookdown-tutorial" into "awesome-book"

*The following is from Jenny Bryan's [Happy Git With R](https://happygitwithr.com/existing-github-last.html)*

1. Go back to RStudio, to your "awesome-book" project
1. In the Console, type `usethis::use_git()` and say Yes to the two prompts. This will restart R and give you a new Git tab in the upper right pane.
1. Now, click on the Terminal tab next to the Console tab. 
1. Type `git remote add origin <paste your copied awesome-book github url here>`
1. Type `git push --set-upstream origin master`



## Publish "awesome-book"

Last steps! 

1. Go back to github.com/username/"awesome-book" and refresh — our files should be there! But we want it to be a book published as https://username.github.io/awesome-book.
1. Click Settings
1. Scroll down to GitHub Settings
1. Change the Source pulldown from "None" to "master branch /docs folder"
1. It should say "Your site is ready to be published at https://username.github.io/awesome-book/" — click the link to see!

Now, you're set — you just need to write your book. 

## Moving forward

As you write your .Rmd files, build the book and commit all files, including the docs/ folder, and your published book will be updated!


## This is to test if it works
As we start coding, the book will be updated after commit
```{r warning=FALSE}
# source("./get_rent_data.R")
# get_rent_data()
```


# Parsing data
The first thing we have to do is to combine the Excel files downloaded from the website. We  then need to parse the data and tidy it up in a format that is ready for analysis. We use a `for` loop to go through all files and read them, then bind them together in one `dataframe`. The `for` loop approach is used so that we can add a field/column to indicate the period of the data (i.e., which quarter and year the data represents). 

```{r}
if(!require(tidyverse)){
  install.packages("tidyverse")} # check and install 'tidyverse' if needed 

library(tidyverse) # load required package

sfiles <- list.files("./RentData", full.names = TRUE) # list all files 

colnm <- c("postcode", "dwt", "beds", "wklyrent_q1","wklyrent_q2","wklyrent_q3", 
           "newbonds","bondnum", "rentchgqrt","rentchgann", "bondchgqrt","bondchgann")
# loop through each file to read and bind them together
res <- temp <- NULL
for (i in 1: length(sfiles)){
    temp <- readxl::read_xlsx(sfiles[i], sheet = 3, 
                              col_names = colnm,  skip = 9) %>% 
        mutate(period = unlist(sfiles[i])) # add the file name to know which quarter
    
    res <- bind_rows(res, temp)
}
rm(temp) # remove temp file which is no longer needed.
```

Before proceeding to data tidying, we need to check the data structure and data types to see if the loop above read the data in correctly. We use the `dplyr::head` function to show the first 6 rows in the `dataframe`. We can see that many fields have a wrong data type (character instead of double). We will change them before formatting it into a so-called `tidy` format.
```{r}
res %>% 
  head()
```

Use `mutate` and `across` functions to convert the data type for multiple fields at once. We convert from character to numeric type. We then use pattern matching to parse the file name and get the corresponding texts that indicate the quarter and the year of the data. 

```{r}
res <- res %>% 
    mutate(across(wklyrent_q1:bondchgann, ~ as.numeric(.)))

res <- res %>% 
    mutate(period = gsub(pattern = "^./Issue-\\d{3}-", "",ignore.case = T,period)) %>% 
    mutate(period = gsub(pattern = "-?./?Rent-tables-", "", ignore.case = T,  period)) %>% 
    mutate(period = gsub(pattern = "-Quarter", "", ignore.case = T, period)) %>% 
    mutate(period = gsub(pattern = "\\.xlsx$", "", period)) %>% 
    mutate(period = gsub(pattern = "./Issue", "",ignore.case = T,period)) %>% 
    mutate(period = gsub(pattern = "./RentDat|./RentDat-\\d{3}", "",
                         ignore.case = T,period)) %>% 
    mutate(period = gsub(pattern = "^-\\d{4}", "",ignore.case = T,period)) %>% 
    separate(period, into = c("quarter", "year"), sep = "-") 

res %>% tail()
```

Many postcodes have no data for weekly rental cost due either to no new rent contracts or small sample size. We can remove these from the dataset. Here, we use `rowSums` to count the total number of rows that has `NA` value and keep only rows where count of `NA` is smaller than 8.
```{r}
res <- res %>% 
    filter(rowSums(is.na(.)) < 8) %>% 
    filter(dwt != "Total" & beds != "Total")
```


```{r}
selacc <- c("1 Bedroom", "2 Bedrooms","3 Bedrooms", "4 or more Bedrooms")

res %>% 
    filter(grepl("^2", postcode)) %>%
    select(postcode, quarter, year, dwt, beds, wklyrent_q2) %>% 
    filter(beds %in% selacc) %>% 
    mutate(month = substr(quarter, 1,3)) %>% 
    mutate(dt2 = paste0(year,"-", quarter, "-01")) %>%
    mutate(qrt = paste0(month,substr(year,3,4))) %>%
    mutate(dt = as.Date(dt2, format = "%Y-%B-%d")) %>% 
    filter(!is.na(wklyrent_q2)) %>% 
    select(postcode, dt, qrt, dwt, beds, wklyrent_q2) %>% 
    arrange(dt) %>% #group_by(qrt) %>% summarise(n = n())
    pivot_wider(id_cols = c("postcode", "dwt", "beds"), 
                names_from = "qrt", 
                values_from = "wklyrent_q2", 
                values_fill = NA) %>% 
    unnest() # to unnest the df within each column.
```




